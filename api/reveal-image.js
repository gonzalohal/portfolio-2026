const { getKv, downloadObject } = require("../lib/supabase");
...
async function readProjects() {
  return (await getKv("projects")) || [];
}

// Live lock status for the "Bloquear portada" feature — reads the projects row fresh
// from Supabase on every request (not any deploy-baked copy), so locking/unlocking a
// project's cover takes effect immediately without a Vercel deploy.
...
// Serves a project's cover image, blurring it on the fly (never touching the file in
// storage) when it's currently locked. A valid preview token bypasses the blur.
async function serveCover(req, res, slug, token) {
  ...
  const storagePath = `work/${slug}/${cover}`;
...
async function serveOriginal(req, res, imgPath, token) {
  ...
  const storagePath = imgPath.replace(/^images\//, "");
  const raw = await downloadObject("site-originals", hiddenOriginalPath(storagePath));
  if (!raw) return res.status(404).json({ error: "No hay original guardado para esta imagen" });
  sendImage(res, storagePath, raw, "private, no-store");
}
